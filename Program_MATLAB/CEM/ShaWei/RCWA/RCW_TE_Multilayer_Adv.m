...
%  编程人：沙威 (Wei E.I. Sha)，香港大学，2009年11月26日
%  Email: wsha@eee.hku.hk

function RCW_TE_Multilayer_Adv;

clc;clear
theta=0*pi/180:pi/180:70*pi/180;  % 入射角度

for index=1:length(theta)
    disp(length(theta)-index)
    [ret_t,ret_r]=Compute(theta(index));  %  计算0阶反射传输系数
    T(index)=(ret_t);
    R(index)=(ret_r);
end
plot(0:1:70,T,'bO-')
...
DE_T=Tra.*conj(Tra).*real(kz2/(k0*n1*cos(theta))).';  %  归一化传输系数
