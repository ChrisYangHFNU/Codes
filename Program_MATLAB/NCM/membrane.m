...
%   Reference:
%   L. Fox, P. Henrici, and C. Moler, Approximations and Bounds for
%   Eigenvalues of Elliptic Operators SIAM J. Numer. Anal. 4, pp. 89-102
%
...
if k > 12
   error(message('MATLAB:membrane:TwelveOnly'))
end
...
t = sqrt(lambda)*r;
b1 = bsxfun(@besselj,alf1,t);
if isempty(alf2)
   b2 = zeros(length(t), length(alf2)); 
else
   b2 = bsxfun(@besselj,alf2,t);
end
A = [b1(:,k1) b2(:,k2)];
[~,k] = sort([k1 k2]);
A = A(:,k);
