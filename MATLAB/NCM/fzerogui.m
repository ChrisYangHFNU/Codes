...
shg
clf reset
set(gcf,'numbertitle','off','name','Fzerogui','color','w');%'menu','none',
done = uicontrol('style','toggle','units','norm','pos',[.03,.01,.10,.05], ...
   'string','手动','callback','set(gcf,''userdata'',1)');
auto = uicontrol('style','toggle','units','norm','pos',[.15,.01,.10,.05], ...
   'string','自动','callback','set(gcf,''userdata'',1)');
uicontrol('style','text','units','norm','back','w','hor','center',...
    'pos',[0.2,0.93,0.6,0.05],'fontsize',10','string',...
    sprintf('Zeroin法求函数 %10s 在 [%.1f,%.1f] 区间的零点',char(F),ab(1),ab(2)))
...
legend([hp(4),hp(7),hp(8)],'结果零点','二分点','逆二插线')
delete(auto)
set(done,'string','关闭','style','push','value',0,'callback','close(gcf)')
...
function [hp,x0] = fzeroplot(hp,F,a,b,c,fa,fb,fc,xbisect,xinterp,varargin)
   ...
   if isempty(hp)
      ...
      Lh=legend([hp(2),hp(8),hp(7),hp(4)],'函数线','插值线','二分点','区间点',...
          'Orientation','hor','position',[0.344,0.0095,0.622,0.0476]);
      if a == c 
