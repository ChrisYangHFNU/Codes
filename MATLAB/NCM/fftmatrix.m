...
% FFTMATRIX(N,J) plots only the J-st column.
% FFTMATRIX with no arguments defaults to FFTMATRIX(10,2).
...
if nargin < 1 || isnumeric(varargin{1})
   ...
   if nargin < 2, j = 2; else j = varargin{2}; end
   shg
   clf reset
   ...
   h(6) = uicontrol('units','norm','pos',[.55 .04 .12 .06], ...
          'style','toggle','fontsize',12,'userdata',j-1, ...
...
set(h(6),'string',['j = ' num2str(j+1)],'userdata',j)
