...
if nargin == 0 || isnumeric(arg1)
   ...
   p(2).Color='r';p(3).Color='g';p(4).Color='k';p(5).Color='b';
   ymin = min(y);
   ymax = max(y);
   ydel = ymax-ymin;
   ...
   uicontrol('units','normal','pos',[.45 .13 .14 .18], ...
...
set(p(2),'xdata',u,'ydata',piecelin(x,y,u),'LineStyle',':','LineWidth',2);
set(p(3),'xdata',u,'ydata',polyinterp(x,y,u),'LineWidth',2);
set(p(4),'xdata',u,'ydata',splinetx(x,y,u),'LineStyle','-.','LineWidth',2);
set(p(5),'xdata',u,'ydata',pchiptx(x,y,u),'LineStyle','--','LineWidth',2);
axis(ax);
