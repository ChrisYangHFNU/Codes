function [t,y]=lorenzgui
%LORENZGUI   输出洛伦茨混沌轨迹或实时绘制围绕吸引子的混沌轨迹
%[t,y]=lorenzgui    输出时间跨度为50的洛伦茨混沌轨迹数据
%lorenzgui          引出GUI交互界面，绘制混沌轨迹　　　
...
if nargout==2
    ...
    A = [ -beta    0     eta
     ...
     y0 = yc + [0; 0; 3]; 
     tspan = [0 50];
     opts = odeset('reltol',1.e-6,'refine',4);
...
if isequal(job,'init')
   ...
   set(gca,'color','black','pos',[.03 .05 .93 .95],'userdata',rho,'View',[75,40])
   axis(R);
   axis off
   comet(1) = animatedline(y(1),y(2),y(3),'marker','.','Color','m',...
      'markersize',0.5);
   comet(2) = line(y(1),y(2),y(3),'marker','.','Color','r',...
       'markersize',25);
   comet(3) = line(NaN,NaN,NaN,'color','y');
   Y = y(:,ones(L,1));

   uics = get(gcf,'children');
   paws = uics(4);
   stop = uics(3);
   ...
   eta = sqrt(beta*(rho-1));
   yc = [rho-1; eta; eta];
   line(yc(1),yc(2),yc(3),'marker','o','color','g','markersize',10)
   line(yc(1),-yc(2),-yc(3),'marker','o','color','g','markersize',10)
...
elseif isequal(job,'done')
   ...
   addpoints(comet(1),Y(1,end),Y(2,end),Y(3,end));
   set(comet(2),'xdata',Y(1,end),'ydata',Y(2,end),'zdata',Y(3,end))
   set(comet(3),'xdata',Y(1,2:end),'ydata',Y(2,2:end),'zdata',Y(3,2:end))
   Y(:,1:L) = [];
   drawnow;
