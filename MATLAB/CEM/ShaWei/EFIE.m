...
for m=1:count  %  场点循环
    for n=1:count  %  源点循环  
        if (m==n)
            l_mn(m,n)=(1-(2*j/pi)*log(EULER*k*circlelength/(4*EX)))*circlelength*(k*Z/4);
            %  奇异点处理
        else
            ...
            integerfunction2=besselj(0,integerfunction1)-j*bessely(0,integerfunction1);
            % % 元素赋值
...
%  电压矩阵g_m确定
for i=1:count;
    g_m(i)=amplitude*exp(-j*k*a*cos(point(i)) );  %  往X方向传播的TM波
end;
...
for m=1:count;
    for n=-20:20;
        initialcurrent=initialcurrent+(2*amplitude/(2*pi*f*U0*pi*a))*...
                       j^(-n)*exp(j*n*point(m))/(besselj(n,k*a)-j*bessely(n,k*a));  %  解析解电流
    end
...
for si=0:pi/180:2*pi;  %  散射角
    ...
    for seg=1:count;
       sum=sum+currentresult(seg,1)*circlelength*exp( j*k*( a*cos(point(seg))*cos(si)+a*sin(point(seg))*sin(si) ) );
    end
