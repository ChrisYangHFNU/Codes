...
\begin{tikzpicture}
    ...
    \begin{smithchart}[
        ...
        \addplot[domain=0:90,samples=600,color=blue]{0.6};
        \addplot+[mark=*,only marks,samples at={0,.02,...,100},
            mark options={solid},color={red},mark size=.2,line width=1](1.7,x);
        \addplot+[mark=*,only marks,samples at={0,-.02,...,-100},
            mark options={solid},color={red},mark size=.2,line width=1](1.7,x);
        \addplot+[mark=*,only marks,samples at={0,.02,...,100},
            mark options={solid},color={blue},mark size=.2,line width=1](2,x);
        \addplot+[mark=*,only marks,samples at={0,-.02,...,-100},
            mark options={solid},color={blue},mark size=.2,line width=1](2,x);
        \coordinate [label=330:$A$] (a) at (1.7,0.6);

        \path[draw=blue,fill=blue] (a) circle (0.1cm);
        
        \path[name path=O-A,draw=blue,thick] (o) -- (a);
        \node (H) [label=135:$EqualReflectionCoefficientCircle$,name path=H,draw=red,thick,circle through={(a)}]at (o) {};
        \coordinate [label=330:$\rho$] (b) at (2,0);
